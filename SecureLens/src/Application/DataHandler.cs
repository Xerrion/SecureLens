...
public class DataHandler
{
    ...
    public List<CompletedUser> BuildCompletedUsers()
    {
        ...
        foreach (AuditLogEntry? auditEntry in _auditLogs)
        {
            var rawAccount = auditEntry.User?.Account;
            if (string.IsNullOrEmpty(rawAccount)) continue;

            var normalizedAccount = NormalizeUserAccount(rawAccount);
        ...
        foreach (InventoryLogEntry? invEntry in _inventoryLogs)
        {
            var rawAccount = invEntry.User?.Account;
            if (string.IsNullOrEmpty(rawAccount)) continue;
        ...
        if (_adRepo != null)
            foreach (KeyValuePair<string, CompletedUser> kvp in _completedUsers)
            {
                var normalizedAccount = kvp.Key; // e.g. "user0192"
                ActiveDirectoryUser adUser = _adRepo.GetAdUserFromFile(normalizedAccount);
                if (adUser != null) kvp.Value.ActiveDirectoryUser = adUser;
            }
    ...
    private string NormalizeUserAccount(string rawAccount)
    {
        ...
        var slashIndex = rawAccount.IndexOf('\\');
        if (slashIndex >= 0 && slashIndex < rawAccount.Length - 1) rawAccount = rawAccount.Substring(slashIndex + 1);
