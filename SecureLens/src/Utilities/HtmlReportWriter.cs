...
public class HtmlReportWriter
{
        ...
        List<AdminByRequestSetting> settings)
    {
        ...
        foreach (AdminByRequestSetting? setting in settings)
            if (stats.MembersPerSetting.TryGetValue(setting.Name, out var memberCount))
                sb.AppendLine(
                    $"    <tr><td>Total members able to use '{setting.Name}':</td><td>{memberCount}</td></tr>");

        sb.AppendLine(
            $"    <tr><td>Users with at least 5 elevations in period:</td><td>{stats.UsersWith5Elevations}</td></tr>");
    ...
    private string GenerateApplicationStatisticsTable(
        Dictionary<string, ApplicationStatisticsResult> appStats)
    {
        if (appStats == null || appStats.Count == 0) return "<p>No application statistics available.</p>";
        ...
        foreach (KeyValuePair<string, ApplicationStatisticsResult> kvp in sortedApps)
        {
            var appName = kvp.Key;
            ApplicationStatisticsResult stats = kvp.Value;
            sb.AppendLine("    <tr>");
    ...
    {
        if (terminalStats == null || terminalStats.Count == 0) return "<p>No terminal usage found.</p>";
        ...
        foreach (TerminalStatisticsRow row in terminalStats)
        {
            sb.AppendLine("    <tr>");
    ...
    private string GenerateUnusedAdGroupsTable(List<UnusedAdGroupResult> unusedGroups)
    {
        ...
        foreach (UnusedAdGroupResult item in sorted)
        {
            sb.AppendLine("    <tr>");
